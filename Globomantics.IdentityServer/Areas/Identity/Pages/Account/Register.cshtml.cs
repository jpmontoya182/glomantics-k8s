...
namespace Globomantics.IdentityServer.Areas.Identity.Pages.Account
{
    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            {
                var user = new CustomUser { LoginName = Input.Email };
                var result = await _userManager.CreateAsync(user, Input.Password);
                if (result.Succeeded)
                {
                    _logger.LogInformation("User created a new account with password.");

                    var newUser = await _userManager.FindByNameAsync(user.LoginName);
                    ...
                    var callbackUrl = Url.Page(
                        ...
                        pageHandler: null,
                        values: new { area = "Identity", userId = newUser.UserId, code, returnUrl },
